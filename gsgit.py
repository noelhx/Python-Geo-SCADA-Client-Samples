...
def get_agg_properties(obj: ObjectDetails, a: str, aggtype: str) -> list[tuple[str, list[tuple[str, Variant]]]]:
    agg_def = interface.get_class( aggtype)
    agg_prop_names = [a + "." + p.name for p in agg_def.properties if p.is_writable and p.property_type == PropertyType.Configuration]
    property_values = []
    this_agg = None
...
def get_properties(obj: ObjectDetails, class_name: str) -> list[tuple[str, list[tuple[str, Variant]]]]:
...
def process_object(obj_id: int, is_new_repo: bool):
    ...
    for obj in object_list:
        data = {'id': obj.id, 'name': obj.name, 'class_name': obj.class_name, 'properties': [], 'doc_content_crc': 0, 'doc_content_len': 0}
